(function () {
  ...
  loginPanel.innerHTML =
    '<div class="wce-discord-login">' +
      '<div><span class="wce-eyebrow">Editorial access</span><h2>Continue with Discord</h2><p>Discord verifies that you are a member of United Marxist Pact. Every server member can read the sources and contribute to the final argument. Everyone else can still read both public article views.</p></div>' +
  ...
  function updateSessionUi() {
    ...
    sessionState.textContent = state.authenticated ? "Signed in as " + name : (state.sessionChecked ? "Public reader" : "Checking access…");
    ...
    finalWorkspace.querySelector(".wce-final-actions").hidden = !state.canEdit;
  ...
  function canViewSourceHistory(message) {
    return Boolean(
      isWebsiteContribution(message) &&
      state.user &&
      (state.user.admin || canEditSourceMessage(message))
    );
  }
